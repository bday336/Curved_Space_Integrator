...
# Convert from hyperboloid model to translational parameterization (H2 velocity)
...
### Generate Initial Conditions for H3 Simulations ###

# The idea is that the position is known and given for each mass that is being
# simulated. The issue then is to find the appropriate initial velocity for the desired
# movement of the mass in the rotational parameterization so that it can be given to the 
# solver to begin the simulation since is the solver functions in the rotational parameteization.
# We use killing fields to generate uniform motion for the quasi rigid extended objects we are
# testing. Currently, the killing field is generated by a loxodromic isometry rotating about the
# x-axis. In the future, we can add a feature to allow the axis to be any arbitrary geodesic in H3.

# The position of the mass first is converted into the hyperboloid implicitly to find the 
# corresponding killing vector at that location. This vector needs to be transformed into the
# basis of the parameterization. This is done through the transforms that have been derived.
# This results in the function returning the initial velocity in the parameterization basis
# corresponding to the uniform motion produced by the killing field. 

def initial_con(pos,trans_vel,rot_vel):
   killing_vec = array([trans_vel*cosh(pos[0]),rot_vel*sinh(pos[0])*cos(pos[1]),-rot_vel*sinh(pos[0])*sin(pos[1])*sin(pos[2]),trans_vel*sinh(pos[0])*sin(pos[1])*cos(pos[2])])
   ad = killing_vec[3]/sinh(pos[0])
   bd = (ad*cosh(pos[0])*cos(pos[1])-killing_vec[2])/(sinh(pos[0])*sin(pos[1]))
