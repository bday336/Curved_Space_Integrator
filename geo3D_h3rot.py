from symint_bank import symh3georot,imph3georot
...
gb=append(gb, abgr[1])
gg=append(gg, abgr[2])
testm.append(symh3georot(posi, posguess, abgdotri, abgdotguess, delT))
testp.append(imph3georot(abgr, abgr, abgdotri, abgdotguess, delT))		
...
while(q < nump-1):
	...
	testm.append(symh3georot(nextposm, nextposm, nextdotm, nextdotm, delT))
	testp.append(imph3georot(nextposp, nextposp, nextdotp, nextdotp, delT))		
...
for j in range(len(checku)):
	...
	lpemerr.append(sqrt(abs((checkx[j]-sinh(ga[j])*sin(gb[j])*cos(gg[j]))**2. + (checky[j]-sinh(ga[j])*sin(gb[j])*sin(gg[j]))**2. + (checkz[j]-sinh(ga[j])*cos(gb[j]))**2. + (checkw[j]-cosh(ga[j]))**2.)))		
