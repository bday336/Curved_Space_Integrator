...
from symint_bank import imph3sprot3,imph3sprot3_condense_econ, imph3sprot_mesh, imph3sprotdis_mesh, imph3sprot_mesh_rigidcon
...
# length of the rigid rods for the edges are calculated from the mesh vertices
# separation values.
# (initial guess at lagrange multipler (all set to 1 here), rest length l_eq)
con_arr=[]
lagmut_arr=[]
for f in mesh[1]:
    con_arr.append([
...
# Make container for the updating velocities
lagmut_arr_copy=cp.deepcopy(lagmut_arr)

# Numerical Integration step
step_data=[
	imph3sprot_mesh_rigidcon(mesh_copy, velocities_copy, delT, masses, lagmut_arr_copy, con_arr, conn_list)
	]
...
new_lagmut_arr=cp.deepcopy(lagmut_arr)
...
lagmut_dat=step_data[0][3*vert_num+3*vert_num:2*3*vert_num+sp_num]
...
# pot_energy=0.
# for h in range(sp_num):
#     pot_energy+=.5*sparr[h][0]*( dist[-1*sp_num+h] - sparr[h][1] )**2.
...
#     step_data=array([
#         imph3sprot_mesh_rigidcon(new_mesh, new_velocities, delT, masses, new_lagmut_arr, con_arr, conn_list)
#         ])

#     # Copy of mesh and velocity array (position of vertices and velocity values will be changed with the integration)
#     new_mesh=cp.deepcopy(new_mesh)
#     new_velocities=cp.deepcopy(new_velocities)
#     new_lagmut_arr=cp.deepcopy(new_lagmut_arr)
...
# np.savetxt("dist_tstep1_data.csv",dist)
