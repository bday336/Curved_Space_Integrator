...
# Longitudinal Initial Velocities
# particles=array([
#     np.concatenate((array([.5,np.pi/2.,0.*np.pi/2.]),initial_con(array([.5,np.pi/2.,0.*np.pi/2.]),2.,.0),[1.,.2])),      #particle 1
#     np.concatenate((array([.5,np.pi/2.,2.*np.pi/2.]),initial_con(array([.5,np.pi/2.,2.*np.pi/2.]),-2.,.0),[1.,.2]))       #particle 2
#     ])

# Parallel Initial Velocities
particles=array([
    np.concatenate((array([.5,np.pi/2.,1.*np.pi/2.]),initial_con(array([.5,np.pi/2.,1.*np.pi/2.]),1.,.0),[1.,.2])),      #particle 1
    np.concatenate((array([.5,np.pi/2.,3.*np.pi/2.]),initial_con(array([.5,np.pi/2.,3.*np.pi/2.]),1.,.0),[1.,.2]))       #particle 2
    ])
...
# Elongation Percent Plot
# ax3=fig.add_subplot(1,3,3)

# ax3.plot(timearr,(dist12-spring_arr[0][1])/spring_arr[0][1]*100.,label="Spring 12 Elongation %")
# # ax3.set_yscale("log",basey=10)
# ax3.set_xlabel('time (s)')	
# ax3.legend(loc='lower right')	
