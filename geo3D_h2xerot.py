from symint_bank import symh2xegeorot,imph2xegeorot
...
gb=append(gb, abgt[1])
gg=append(gg, abgt[2])
testm.append(symh2xegeorot(posi, posguess, abgdotri, abgdotguess, delT))
testp.append(imph2xegeorot(abgr, abgr, abgdotri, abgdotguess, delT))		
...
while(q < nump-1):
	...
	testm.append(symh2xegeorot(nextposm, nextposm, nextdotm, nextdotm, delT))
	testp.append(imph2xegeorot(nextposp, nextposp, nextdotp, nextdotp, delT))		
...
for b in range(len(gx)):
    ...
    gs=append(gs,gz[b]) 
    gpu=append(gpu,sinh(ga[b])*cos(gb[b])/(cosh(ga[b]) + 1.))
    gpv=append(gpv,sinh(ga[b])*sin(gb[b])/(cosh(ga[b]) + 1.))
    gps=append(gps,gg[b]) 	    	     
...
checkhdata=[]
checkemdata=[]
liemag=array([sqrt(abgdotri[0]**2. + sinh(abgr[0])**2.*abgdotri[1]**2.),0.,0.])
for c in timearr:
...
for d in range(len(timearr)):
	...
	checks.append(abgr[2]+abgdotri[2]*timearr[d])
	checkx.append(checkemdata[d][0])
	checky.append(checkemdata[d][1])
	checkz.append(abgr[2]+abgdotri[2]*timearr[d])
...
for j in range(len(checku)):
	spemerr.append(sqrt((gx[j]-sinh(ga[j])*cos(gb[j]))**2. + (gy[j]-sinh(ga[j])*sin(gb[j]))**2. + (gz[j]-gg[j])**2. + (gw[j]-cosh(ga[j]))**2.))
	slemerr.append(sqrt((gx[j]-checkx[j])**2. + (gy[j]-checky[j])**2. + (gz[j]-checkz[j])**2.+ (gw[j]-checkw[j])**2.))
	lpemerr.append(sqrt(abs((checkx[j]-sinh(ga[j])*cos(gb[j]))**2. + (checky[j]-sinh(ga[j])*sin(gb[j]))**2. + (checkz[j]-gg[j])**2. + (checkw[j]-cosh(ga[j]))**2.)))		
