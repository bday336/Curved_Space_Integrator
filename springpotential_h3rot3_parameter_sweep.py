...
for v in arange(1.,7.,1.): # max for each was set to 11 (1,11,1)
    for l in arange(1.,7.,1.): # 10 entries (1,11,1)
        for k in arange(0,7.,1.): # 11 entries (0,11,1)
            ...
            particles=array([
                np.concatenate((array([arccosh(cosh(l)/cosh(l/2.)),np.pi/2.,0.]),initial_con(array([arccosh(cosh(l)/cosh(l/2.)),np.pi/2.,0.]),v,.0),[1.,.2])),       #particle 1
                np.concatenate((array([l/2.,np.pi/2.,1.*np.pi/2.]),initial_con(array([l/2.,np.pi/2.,1.*np.pi/2.]),v,.0),[1.,.2])),                                   #particle 2
                np.concatenate((array([l/2.,np.pi/2.,3.*np.pi/2.]),initial_con(array([l/2.,np.pi/2.,3.*np.pi/2.]),v,.0),[1.,.2]))                                    #particle 3
                ])
